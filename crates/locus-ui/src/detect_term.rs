...
/// Check if terminal is one of known unsupported terminals
...
impl AdaptiveColors {
    ...
    /// Get blue color - RGB for supported terminals, standard for unsupported
    pub fn blue() -> Color {
        if should_use_rgb_colors() {
            Color::Rgb(60, 120, 216) // Nice blue color
        } else {
            Color::Blue
        }
    }

    /// Get yellow color - RGB for supported terminals, standard for unsupported
    pub fn yellow() -> Color {
        if should_use_rgb_colors() {
            Color::Rgb(229, 192, 123) // Nice yellow color
        } else {
            Color::Yellow
        }
    }

    /// Get cyan color - RGB for supported terminals, standard for unsupported
    pub fn cyan() -> Color {
        if should_use_rgb_colors() {
            Color::Rgb(86, 182, 194) // Nice cyan color
        } else {
            Color::Cyan
        }
    }
    ...
    pub fn red() -> Color {
        if should_use_rgb_colors() {
            Color::Rgb(224, 108, 117) // Nice red color
        } else {
            Color::Red
        }
    }
