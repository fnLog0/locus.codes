...
impl ClientBuilder {
    ...
    pub fn with_inference_config(mut self, inference_config: InferenceConfig) -> Self {
        ...
        // Register LocusCode if configured
        if let Some(config) = inference_config.locus_code_config
            && let Ok(provider) = LocusCodeProvider::new(config)
        {
            registry = registry.register("locus_code", provider);
        }
