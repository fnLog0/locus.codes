...
use crate::providers::{
    anthropic::AnthropicConfig, gemini::GeminiConfig, locus_code::LocusCodeProviderConfig,
    openai::OpenAIConfig,
};
...
///         .locus_code("your-locus_code-key", None)
/// );
/// ```
...
pub struct InferenceConfig {
    ...
    pub(crate) locus_code_config: Option<LocusCodeProviderConfig>,
...
impl InferenceConfig {
    ...
    /// Configure LocusCode provider with API key and optional base URL
    ///
    /// LocusCode provides unified access to multiple LLM providers through
    ...
    /// # use stakai::InferenceConfig;
    /// let config = InferenceConfig::new()
    ///     .locus_code("your-api-key", None);
    ///
    /// // With custom base URL
    /// let config = InferenceConfig::new()
    ///     .locus_code("your-api-key", Some("https://custom.locus_code.dev".to_string()));
    /// ```
    pub fn locus_code(mut self, api_key: impl Into<String>, base_url: Option<String>) -> Self {
        let mut config = LocusCodeProviderConfig::new(api_key);
        if let Some(url) = base_url {
            config = config.with_base_url(url);
        }
        self.locus_code_config = Some(config);
        self
    }

    /// Configure LocusCode provider with full LocusCodeProviderConfig
    ///
    /// # Example
    ///
    /// ```rust,no_run
    /// # use stakai::{InferenceConfig, providers::locus_code::LocusCodeProviderConfig};
    /// let locus_code_config = LocusCodeProviderConfig::new("your-api-key")
    ///     .with_base_url("https://custom.locus_code.dev");
    ///
    /// let config = InferenceConfig::new()
    ///     .locus_code_config(locus_code_config);
    /// ```
    pub fn locus_code_config(mut self, config: LocusCodeProviderConfig) -> Self {
        self.locus_code_config = Some(config);
        self
    }
}
