...
pub fn edit_file_diff_lines(
    ...
    let additions = rows
        .iter()
        .filter(|r| matches!(r.change, crate::diff::ChangeType::Added))
        .count();
    let removals = rows
        .iter()
        .filter(|r| matches!(r.change, crate::diff::ChangeType::Removed))
        .count();
    ...
    let last_old = rows
        .iter()
        .rev()
        .find_map(|r| r.old_line_no.or(r.new_line_no));
    let first_new = rows.iter().find_map(|r| r.new_line_no);
    let last_new = rows
        .iter()
        .rev()
        .find_map(|r| r.new_line_no.or(r.old_line_no));
    ...
    lines.push(diff_header_line(
        path, additions, removals, range, None, palette, width,
    ));
...
mod tests {
    ...
    fn diff_lines_have_header_and_footer() {
        let palette = LocusPalette::locus_dark();
        let lines =
            edit_file_diff_lines("src/main.rs", "old line\n", "new line\n", &palette, 80, 12);
        assert!(
            lines
                .first()
                .unwrap()
