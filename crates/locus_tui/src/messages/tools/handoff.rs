...
pub fn handoff_status_summary(
    ...
    vec![Span::styled(truncated, text_muted_style(palette.text_muted))]
}
...
fn truncate_goal(s: &str) -> String {
    ...
    if trimmed.chars().count() <= MAX_GOAL_LEN {
        ...
        let t: String = trimmed.chars().take(MAX_GOAL_LEN.saturating_sub(1)).collect();
        format!("{}…", t)
    }
}
...
mod tests {
    ...
    fn preview_shows_summary() {
        ...
        assert!(line.spans.iter().any(|s| s.content.contains("Modified 3 files")));
    }
}
