...
pub fn tool_group_header_line(
    ...
    let (status_icon, status_style) = if running_count > 0 {
        (running_indicator.unwrap_or(TOOL_RUNNING_INDICATOR), text_style(palette.accent))
    ...
    let mut spans = vec![
        ...
        Span::styled("Tools".to_string(), text_style(palette.text).add_modifier(Modifier::BOLD)),
        Span::raw("  "),
...
pub fn tool_call_lines(
    ...
    if in_group {
        line1.push(Span::styled(tree_connector.to_string(), text_muted_style(palette.text_muted)));
    }
    ...
    match &msg.status {
        ...
        ToolCallStatus::Error { message } => {
            ...
            if let Some(summary) = &msg.summary {
                lines.push(tool_detail_line(&error_prefix, palette, summary.clone(), false));
            }
            lines.push(tool_detail_line(&error_prefix, palette, message.clone(), true));
            lines
        }
    }
}
...
mod tests {
    ...
    fn tool_call_grouped_indent() {
        ...
        assert!(lines[0].spans.iter().any(|s| s.content.contains("├──") || s.content.contains("└──")));
    }
