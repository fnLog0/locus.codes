...
use locus_code_shared::models::integrations::openai::AgentModel;
use uuid::Uuid;

pub fn get_locus_code_version() -> String {
...
pub fn push_status_message(state: &mut AppState) {
    ...
    let version = get_locus_code_version();
    ...
    let lines = vec![
        Line::from(vec![Span::styled(
            format!("LocusCode Status v{}", version),
            Style::default()
                .fg(Color::Reset)
...
pub fn push_help_message(state: &mut AppState) {
    ...
    let usage_modes = vec![
        ("REPL", "locus_code (interactive session)", Color::Reset),
        (
            "Non-interactive",
            "locus_code -p  \"prompt\" -c <checkpoint_id>",
            Color::Reset,
        ),
    ];
    ...
    let shortcuts = vec![
        ...
        ("Ctrl+C", "quit LocusCode", Color::Yellow),
    ];
...
pub fn push_issue_message(state: &mut AppState) {
    let url = "https://github.com/locus-code/cli/issues/new";
