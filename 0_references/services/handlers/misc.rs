...
use locus_code_shared::models::llm::LLMModel;
use uuid::Uuid;
...
pub fn handle_error(state: &mut AppState, err: String) {
    if err.contains("FREE_PLAN") {
        ...
        push_error_message(
            state,
            "Please top up your account at https://locus_code.dev/settings/billing to keep LocusCoding.",
            Some(true),
        );
        return;
    }
...
pub fn handle_billing_info_loaded(
    state: &mut AppState,
    billing_info: locus_code_shared::models::billing::BillingResponse,
) {
