...
/// Collects volumes from config and always appends locus_code config if it exists and isn't already mounted
...
fn prepare_volumes(config: &AppConfig, check_enabled: bool) -> Vec<String> {
    ...
    // Always append locus_code config and auth files if they exist and not already in the list
    if let Ok(home_dir) = std::env::var("HOME") {
        let locus_code_dir = Path::new(&home_dir).join(".locus_code");

        // Mount config.toml if it exists
        let config_path = locus_code_dir.join("config.toml");
        if config_path.exists() {
            let config_path_str = config_path.to_string_lossy();
            let locus_code_config_mount =
                format!("{}:/home/agent/.locus_code/config.toml:ro", config_path_str);

            // Check if locus_code config is already in the volume list
            let config_already_mounted = volumes_to_mount.iter().any(|v| {
                v.contains("/.locus_code/config.toml")
                    || v.ends_with(":/home/agent/.locus_code/config.toml:ro")
                    || v.ends_with(":/home/agent/.locus_code/config.toml")
            });

            if !config_already_mounted {
                volumes_to_mount.push(locus_code_config_mount);
            }
        }

        // Mount auth.toml if it exists (contains provider credentials)
        let auth_path = locus_code_dir.join("auth.toml");
        if auth_path.exists() {
            let auth_path_str = auth_path.to_string_lossy();
            let locus_code_auth_mount = format!("{}:/home/agent/.locus_code/auth.toml:ro", auth_path_str);

            // Check if auth.toml is already in the volume list
            let auth_already_mounted = volumes_to_mount.iter().any(|v| {
                v.contains("/.locus_code/auth.toml")
                    || v.ends_with(":/home/agent/.locus_code/auth.toml:ro")
                    || v.ends_with(":/home/agent/.locus_code/auth.toml")
            });

            if !auth_already_mounted {
                volumes_to_mount.push(locus_code_auth_mount);
            }
        }
    }
...
pub async fn run_default_warden(
    ...
    // Use locus_code image with current CLI version
    let locus_code_image = format!(
        "ghcr.io/locus_code/agent-warden:v{}",
        env!("CARGO_PKG_VERSION")
    );
    cmd.args(["--image", &locus_code_image]);
...
/// Re-execute the locus_code command inside warden container
pub async fn run_locus_code_in_warden(config: AppConfig, args: &[String]) -> Result<(), String> {
    // Get warden path (will download if not available)
    let warden_path = get_warden_plugin_path().await;
    ...
    // Build the locus_code command to run inside container
    // We need to reconstruct the original command but add LOCUS_CODE_SKIP_WARDEN to prevent recursion
    let mut locus_code_args = vec!["locus_code".to_string()];

    // Skip the first arg (program name) and add the rest
    for arg in args.iter().skip(1) {
        locus_code_args.push(arg.clone());
    }

    // Set environment variable to prevent infinite recursion
    cmd.args(["--env", "LOCUS_CODE_SKIP_WARDEN=1"]);

    // If profile was specified, pass it through
    if let Ok(profile) = std::env::var("LOCUS_CODE_PROFILE") {
        cmd.args(["--env", &format!("LOCUS_CODE_PROFILE={}", profile)]);
    }

    // Pass through API key if set
    if let Ok(api_key) = std::env::var("LOCUS_CODE_API_KEY") {
        cmd.args(["--env", &format!("LOCUS_CODE_API_KEY={}", api_key)]);
    }

    // Pass through API endpoint if set
    if let Ok(api_endpoint) = std::env::var("LOCUS_CODE_API_ENDPOINT") {
        cmd.args(["--env", &format!("LOCUS_CODE_API_ENDPOINT={}", api_endpoint)]);
    }

    // Join all locus_code arguments into a single command string, properly escaping arguments
    let locus_code_cmd = locus_code_args
        .iter()
        .map(|arg| shell_escape_arg(arg))
        .collect::<Vec<_>>()
        .join(" ");
    cmd.arg(locus_code_cmd);
