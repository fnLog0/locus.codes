...
use locus_code_api::{AgentClient, AgentClientConfig, AgentProvider, LocusCodeConfig, LocusGraphConfig};
...
pub enum Commands {
    /// Get CLI Version
    Version,
    /// Login to LocusCode (DEPRECATED: use `locus_code auth login -p locus_code` instead)
    #[command(hide = true)]
    Login {
        /// API key for authentication
        #[arg(long, env("LOCUS_CODE_API_KEY"))]
        api_key: String,
    },

    /// Logout from LocusCode (DEPRECATED: use `locus_code auth logout -p locus_code` instead)
    #[command(hide = true)]
    Logout,
    ...
    Set {
        ...
        /// Enable or disable auto-appending .locus_code to .gitignore files
    ...
    /// LocusGraph graph management commands
    #[command(subcommand)]
    Graph(GraphCommands),
    ...
    /// LocusCode Warden wraps coding agents to apply security policies and limit their capabilities
    Warden {
    ...
    /// Update LocusCode Agent to the latest version
    Update,
}
...
async fn get_client(config: &AppConfig) -> Result<Arc<dyn AgentProvider>, String> {
    ...
    let locus_code = config.get_locus_code_api_key().map(|api_key| LocusCodeConfig {
        api_key,
    ...
    let client = AgentClient::new(AgentClientConfig {
        locus_code,
        locusgraph: config.locusgraph.as_ref().map(|lg| LocusGraphConfig {
            base_url: lg.base_url.clone(),
            token: lg.token.clone(),
            agent_id: lg.agent_id.clone(),
            graph_id: lg.graph_id.clone(),
        }),
        providers,
    ...
    pub async fn run(self, config: AppConfig) -> Result<(), String> {
        match self {
            ...
            Commands::Login { api_key } => {
                ...
                eprintln!("\x1b[33mWarning: 'locus_code login' is deprecated.\x1b[0m");
                eprintln!("Please use: \x1b[1;34mlocus_code auth login --provider locus_code\x1b[0m");
                eprintln!();
            ...
            Commands::Logout => {
                // Show deprecation warning
                eprintln!("\x1b[33mWarning: 'locus_code logout' is deprecated.\x1b[0m");
                eprintln!("Please use: \x1b[1;34mlocus_code auth logout --provider locus_code\x1b[0m");
                eprintln!();
            ...
            Commands::Set {
                ...
                if let Some(append) = auto_append_gitignore {
                    ...
                    config_updated = true;
                    println!("Auto-appending .locus_code to .gitignore: {}", append);
                }
                ...
                if config_updated {
                    ...
                    println!(
                        "  --auto-append-gitignore <bool>  Enable/disable auto-appending .locus_code to .gitignore"
                    );
                }
            }
            Commands::Config(config_command) => {
                match config_command {
                    ...
                    ConfigCommands::Show => {
                        ...
                        println!(
                            "  Auto-append .locus_code to .gitignore: {}",
            ...
            Commands::Version => {
                println!(
                    "locus_code v{} (https://github.com/locus_code/agent)",
            ...
            Commands::Graph(graph_command) => {
                graph_command.run(config).await?;
            }
            Commands::Skills(skills_command) => {
                skills_command.run(config).await?;
            }
            Commands::Acp { system_prompt_file } => {
                ...
                let agent =
                    match crate::commands::acp::LocusCodeAcpAgent::new(config, tx, system_prompt)
                        .await
                    {
                        Ok(agent) => agent,
                        Err(e) => {
...
fn print_sample_config() {
    println!(
        r#"# LocusCode Configuration File
...
[profiles.all]
api_endpoint = "https://apiv2.locus_code.dev"
...
include = ["locus_code://yourdomain.com/common/**"]
exclude = ["locus_code://yourdomain.com/archive/**"]
...
include = ["locus_code://yourdomain.com/*", "locus_code://**/*.md"]

# URI patterns to exclude (supports glob patterns)
exclude = ["locus_code://restricted.domain.com/**"]
...
# When enabled, the main 'locus_code' command will automatically run with Warden security enforcer
...
include = ["locus_code://yourdomain.com/prod/**"]
exclude = ["locus_code://yourdomain.com/dev/**", "locus_code://yourdomain.com/test/**"]
...
include = ["locus_code://yourdomain.com/dev/**", "locus_code://yourdomain.com/test/**"]
exclude = []
...
# Automatically append .locus_code to .gitignore files
...
/// Re-execute locus_code with a specific profile
fn re_execute_locus_code_with_profile(profile: &str, config_path: Option<&std::path::Path>) {
    let mut cmd = Command::new("locus_code");
