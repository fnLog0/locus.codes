...
use clap::Subcommand;
use locus_code_mcp_server::ToolMode;
...
fn find_mcp_proxy_config_file() -> Result<String, String> {
    // Priority 1: ~/.locus_code/mcp.{toml,json}
    let config_path = AppConfig::get_config_path::<&str>(None);
    if let Some(home_locus_code) = config_path.parent() {
        let home_toml = home_locus_code.join("mcp.toml");
        if home_toml.exists() {
            return Ok(home_toml.to_string_lossy().to_string());
        }

        let home_json = home_locus_code.join("mcp.json");
        if home_json.exists() {
            return Ok(home_json.to_string_lossy().to_string());
        }
    }

    // Priority 2: .locus_code/mcp.{toml,json} in current directory
    let cwd_locus_code = PathBuf::from(".locus_code");

    let cwd_locus_code_toml = cwd_locus_code.join("mcp.toml");
    if cwd_locus_code_toml.exists() {
        return Ok(cwd_locus_code_toml.to_string_lossy().to_string());
    }

    let cwd_locus_code_json = cwd_locus_code.join("mcp.json");
    if cwd_locus_code_json.exists() {
        return Ok(cwd_locus_code_json.to_string_lossy().to_string());
    }
    ...
    Err("No MCP proxy config file found. Searched in:\n  \
        1. ~/.locus_code/mcp.toml or ~/.locus_code/mcp.json\n  \
        2. .locus_code/mcp.toml or .locus_code/mcp.json\n  \
