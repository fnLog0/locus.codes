...
use locus_code_api::models::ApiStreamError;
use locus_code_api::{AgentClient, AgentClientConfig, AgentProvider, LocusCodeConfig, LocusGraphConfig};
use locus_code_mcp_client::McpClient;
use locus_code_shared::models::integrations::mcp::CallToolResultExt;
use locus_code_shared::models::integrations::openai::{
...
use locus_code_shared::models::llm::LLMTokenUsage;
use std::cell::Cell;
use std::path::Path;
use std::sync::Arc;
...
impl LocusCodeAcpAgent {
    pub async fn new(
        ...
        let client: Arc<dyn AgentProvider> = {
            let locus_code_api_key = config.get_locus_code_api_key();
            if locus_code_api_key.is_none() {
                log::warn!("No LocusCode API key found. Running in local mode.");
            }

            // Use credential resolution with auth.toml fallback chain
            let locus_code = locus_code_api_key.map(|api_key| LocusCodeConfig {
                api_key,
    ...
    pub async fn initialize_mcp_server_and_tools(
        ...
        let mcp_client = Arc::new(
            locus_code_mcp_client::connect(None) // progress_tx will be set later in run_stdio
                .await
                .map_err(|e| format!("Failed to connect to MCP proxy: {}", e))?,
        );

        // Get tools from MCP client
        let mcp_tools = locus_code_mcp_client::get_tools(&mcp_client)
            .await
    ...
    pub async fn run_stdio(&self) -> Result<(), String> {
        ...
        local_set
            .run_until(async move {
                ...
                // Start up the LocusCodeAcpAgent connected to stdio.
                let (conn, handle_io) =
...
impl Clone for LocusCodeAcpAgent {
    fn clone(&self) -> Self {
        Self {
            config: self.config.clone(),
            client: self.client.clone(),
...
impl acp::Agent for LocusCodeAcpAgent {
    async fn initialize(
        &self,
        ...
        let auth_methods = if self.config.api_key.is_none() {
            vec![acp::AuthMethod::new(
                acp::AuthMethodId::new("locus_code"),
                "Login to LocusCode",
            )
            .description("Authenticate via browser to get your LocusCode API key. A browser window will open for you to sign in.")]
        } else {
            Vec::new()
        };
    ...
    async fn authenticate(
        ...
        // Handle LocusCode authentication via browser redirect (ACP Agent Auth)
        if method_id == "locus_code" {
            log::info!("LocusCode auth method selected, initiating browser-based authentication");
        ...
        // Legacy support: check for LOCUS_CODE_API_KEY environment variable
        if method_id == "github" {
            log::info!(
                "Legacy github auth method selected, checking for LOCUS_CODE_API_KEY environment variable"
            );
            match std::env::var("LOCUS_CODE_API_KEY") {
                Ok(_api_key) => {
                    log::info!("LOCUS_CODE_API_KEY found in environment");
                    return Ok(acp::AuthenticateResponse::new());
                }
                Err(_) => {
                    log::error!("LOCUS_CODE_API_KEY environment variable is not set");
                    return Err(
                        acp::Error::auth_required().data("LOCUS_CODE_API_KEY is not set. Use the 'locus_code' auth method for browser-based authentication.".to_string())
                    );
                }
            }
        }
    ...
    async fn new_session(
        ...
        let has_api_key =
            self.config.api_key.is_some() || std::env::var("LOCUS_CODE_API_KEY").is_ok() || {
        ...
        if !has_api_key {
            ...
            return Err(acp::Error::auth_required().data(
                "Authentication required. Use the 'locus_code' auth method to authenticate via browser.".to_string()
            ));
        }
