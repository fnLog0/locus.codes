...
pub async fn run_auto_update() -> Result<(), String> {
    ...
    if is_homebrew_installed()
        && is_locus_code_homebrew_install()
...
fn is_locus_code_homebrew_install() -> bool {
...
fn update_via_brew() -> Result<(), String> {
    ...
    if upgrade_status.success() {
        ...
        Err("brew upgrade locus_code failed".to_string())
    }
}
...
fn is_current_binary_homebrew_managed() -> Result<bool, String> {
    ...
    // Get Homebrew's locus_code path
    match Command::new("brew").arg("--prefix").arg("locus_code").output() {
        ...
        Ok(_) => {
            // brew --prefix locus_code failed (locus_code not installed via brew)
            Ok(false)
        }
        Err(_) => {
...
async fn update_binary_atomic(os: &str, arch: &str, version: Option<String>) -> Result<(), String> {
    ...
    let cli_name = "locus_code";
    let base_url = "https://github.com/locus_code/agent/releases/download";
