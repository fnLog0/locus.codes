...
use locus_code_api::AgentProvider;
use locus_code_shared::cert_utils::CertificateChain;
use locus_code_shared::models::subagent::SubagentConfigs;
use locus_code_shared::task_manager::{TaskManager, TaskManagerHandle};
...
impl AuthConfig {
    pub async fn new(disabled: bool) -> Self {
        let token = if disabled {
            ...
            let token = locus_code_shared::utils::generate_password(64, true);
            Some(token)
        };
    ...
    if redact_secrets {
        tokio::spawn(async move {
            match std::panic::catch_unwind(|| {
                locus_code_shared::secrets::initialize_gitleaks_config(privacy_mode)
            }) {
                Ok(_rule_count) => {}
                Err(_) => {
...
async fn create_shutdown_handler(
    ...
    task_manager_handle: Option<std::sync::Arc<locus_code_shared::task_manager::TaskManagerHandle>>,
) {
