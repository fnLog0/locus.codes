...
use locus_code_shared::file_backup_manager::FileBackupManager;
use locus_code_shared::remote_connection::{
...
use locus_code_shared::models::integrations::openai::ToolCallResultProgress;
use locus_code_shared::task_manager::TaskInfo;
use locus_code_shared::tls_client::{TlsClientConfig, create_tls_client};
use locus_code_shared::utils::{
...
"
    ...
    pub async fn generate_password(
        ...
        let password = locus_code_shared::utils::generate_password(length, no_symbols);
...
The response will be truncated if it exceeds 300 lines, with the full content saved to a local file."
    ...
    pub async fn view_web_page(
        ...
        headers.insert(
            USER_AGENT,
            HeaderValue::from_static("Mozilla/5.0 (compatible; LocusCode-MCP-Bot/1.0)"),
        );
...
- Local files: Moved to '.locus_code/session/backups/{uuid}/' on the local machine
- Remote files: Moved to '.locus_code/session/backups/{uuid}/' on the remote machine
...
- Use backup paths from XML output to restore files if needed"
    ...
    async fn wait_for_tasks_with_streaming(
        ...
        ctx: &RequestContext<RoleServer>,
    ) -> Result<Vec<TaskInfo>, locus_code_shared::task_manager::TaskError> {
        ...
        if !missing_tasks.is_empty() {
            return Err(locus_code_shared::task_manager::TaskError::TaskNotFound(
            ...
            loop {
                ...
                for task_id in task_ids {
                    if let Some(task) = all_tasks.iter().find(|t| &t.id == task_id) {
                        match task.status {
                            locus_code_shared::task_manager::TaskStatus::Pending
                            | locus_code_shared::task_manager::TaskStatus::Running => {
        ...
        if let Some(timeout_duration) = timeout {
            match tokio_timeout(timeout_duration, wait_operation).await {
                Ok(result) => result,
                Err(_) => Err(locus_code_shared::task_manager::TaskError::TaskTimeout),
            }
        } else {
